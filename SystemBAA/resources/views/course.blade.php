    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
          <div class="x_panel">
            ...
            <div class="col-xs-8 col-xs-offset-2">
                <form action="/course" method="get" id="searchForm" class="input-group">
              ...
              <tbody>
                @foreach ($table as $key)
                  <tr>
                    ...
                    @endforeach</td>
                    <td>{{$key->sks}}</td>
                    <td> <button class="btn btn-success" onclick=" window.open('{{env('URL_KRS')}}/peserta?id={{$key->id}}','_blank')"> Peserta</button></td>
                    <td> <button class="btn btn-success" onclick=" window.open('{{env('URL_KHS')}}/input?id={{$key->id}}','_blank')"> Nilai</button></td>
                    <td> <button class="btn btn-success" onclick=" window.open('{{env('URL_KRS')}}/edit?id={{$key->id}}','_blank')"> Edit</button></td>
                  </tr>
                @endforeach
